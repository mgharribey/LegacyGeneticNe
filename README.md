...
This repository contains the main scripts and associated workflows described in the publication [Harribey et al. *in prep.*](*LINK TO ADD*)



## Construction of the categorization LHT variation plane framework

Starting from the life-history traits (LHT) variation plan described in different studies among which Capdevila et al. 2021, we tested the robustness of the species coordinates and thus of trait correlations to principal axes. 
...
Filtering steps were applied to SNP datasets in VCF format (Danecek et al. 2011) to homogeneise and ensure the quality of data and to apply current recommendations regarding genetic estimates of Ne (Hemstrom et al. 2024; Waples 2024).
...
- Estimates of [isolation-by-distance](https://mgharribey.github.io/LegacyGeneticNe/docs/IBD_calculation.html) using the Rousset (1997) approach


### 3. Estimations of contemporary Ne

Estimation of effective population size using NeLD in NeEstimator (Do et al. 2014) in different populations across datasets has been implemented in an [automatic way for SSR](https://mgharribey.github.io/LegacyGeneticNe/docs/NeEstims_withNeLD_SSR.html), using RLDNe R package (Robinson 2019). 

The same [pipeline adpated for SNP](https://mgharribey.github.io/LegacyGeneticNe/docs/NeEstims_withNeLD_SNP.html) also includes the creation of replicates subsetting the number of loci when their number is sufficient and a correction for pseudoreplication (Waples et al. 2016). 

We also used two other software to compare the estimation of Ne with NeLD: Estim for SSR and ONeSAMP 3.0 for SNPs (not included in the Galaxy-Ecology workflow). 

You may find [scripts used for running ONeSAMP 3.0](https://mgharribey.github.io/LegacyGeneticNe/docs/NeEstims_withONeSAMP3.html) useful. They include a reencoding SNP datasets.
